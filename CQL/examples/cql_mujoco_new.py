...
def load_hdf5(dataset, replay_buffer, buffersize = 100000):
    replay_buffer._observations = dataset['observations'][:buffersize]
    replay_buffer._next_obs = dataset['next_observations'][:buffersize]
    replay_buffer._actions = dataset['actions'][:buffersize]
    replay_buffer._rewards = np.expand_dims(np.squeeze(dataset['rewards']), 1)[:buffersize]
    replay_buffer._terminals = np.expand_dims(np.squeeze(dataset['terminals']), 1)[:buffersize]
    replay_buffer._size = dataset['terminals'][:buffersize].shape[0]
...
def experiment(variant):
    ...
    if variant['load_buffer'] and buffer_filename is not None:
        ...
        load_hdf5(d4rl.qlearning_dataset(eval_env), replay_buffer, variant['replay_buffer_size'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--conservative_factor', default=0.0, type=float)
    ...
    variant['trainer_kwargs']['conservative_factor'] = args.conservative_factor
